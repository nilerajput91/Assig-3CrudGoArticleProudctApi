...
import (
	...
	"github.com/nilerajput91/Assig-3CrudGoArticleProudctApi/api/responses"
	"github.com/nilerajput91/Assig-3CrudGoArticleProudctApi/api/utils/formaterror"
)
...
func (server *Server) CreateArticle(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Location", fmt.Sprintf("%s%s/%d", r.Host, r.URL.Path, articleCreated.ID))
...
func (server *Server) GetArticles(w http.ResponseWriter, r *http.Request) {
	...
	articles, err := article.FindAllArticles(server.DB)
	if err != nil {
